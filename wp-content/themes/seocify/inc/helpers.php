...
if ( !function_exists( 'seocify_content_read_more' ) ) {
	...
	function seocify_content_read_more( $num = 20, $button = true ) {
		$blog_button_title		 = seocify_option( 'blog_button_title' );

		if (empty($blog_button_title)) {
			$blog_button_title = esc_html__( 'Continue Reading', 'seocify' );
		} else {
			$blog_button_title = seocify_option( 'blog_button_title' );
		}
		...
		echo '</div>';
		if ( $button == true ) {
			echo '<div class="btn-wraper"><a href="' . get_the_permalink() . '" class="btn btn-primary icon-right">' . esc_html( $blog_button_title ) . '<i class="icon icon-arrow-right"></i></a></div>';
		}
	}
